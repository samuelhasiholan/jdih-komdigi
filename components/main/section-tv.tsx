...
export interface SectionTvProps {
  ...
  openModalVideoPlayer: (title: string, video: VideoInterface | null) => void;
}
    ...
    <section className="main-section flex flex-col py-11">
      ...
      <div className="flex mb-6">
        <span className="flex-auto text-title">
          Saksikan tayangan seputar JDIH Kemkomdigi yang berisi seputar
          penjelasan secara interaktif tentang JDIH Kemkominfo.
        </span>
        <button
        ...
        {isLoading ? (
          ...
          data?.map((value, index) => (
            <Button
              ...
              key={index}
              onClick={() => props.openModalVideoPlayer(value.judul, value)}
            >
              ...
              <Image
                ...
                alt="pos"
                radius="none"
                src={process.env.NEXT_PUBLIC_FILE_URL + "/" + value.previewPath}
                removeWrapper
              />
              <div className="text-left text-small">
                ...
                <p className="whitespace-normal">{value?.createdAt}</p>
              </div>
            </Button>
          ))
        )}
